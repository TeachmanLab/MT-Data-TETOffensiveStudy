# ---------------------------------------------------------------------------- #
# Redact Data
# Authors: Jeremy W. Eberle and Kaitlyn Petz (for TET checks)
# ---------------------------------------------------------------------------- #
...
# from "1_get_raw_data.ipynb" are in "./data/1_raw_calm_full". This script will import 
# raw data from "./data/1_raw_calm_full" and output redacted data to "./data/2_redacted".
...
# Scope: This script was based on data dumped from "calm" database on the Data
...
## Kaitlyn: We can see that we are running on the newer version of R (v4.2.3, 2023-03-15). 
  # Maria: warning used to note that script was based on R version 4.0.3. 
  # On 9/26/23 we changed this to be R (v 4.2.3, 2023-03-15) instead.
    # Kaitlyn checked that script ran the same/led to same outcomes in R v4.0.3 as R v4.2.3
...
raw_data_dir <- paste0(wd_dir, "/data/1_raw_calm_full")
filenames <- list.files(raw_data_dir, pattern = "*.csv", full.names = FALSE)

## Check that the correct number of files were imported, there should be 67
length(filenames) == 67
...
## Check that there are rows in rows1, meaning we have grabbed entries that are fill in the blank
nrow(rows1)!=0
...
## Check that there are rows in rows2, meaning we have grabbed entries that are fill in the blank
nrow(rows2)!=0
...
## Check that there are rows in rows3, meaning we have correctly grabbed the flexible thinking responses (which are FITB)
nrow(rows3)!=0
...
## Check that there are rows in rows4_all_conditions, meaning we have grabbed the write your own entries
nrow(rows4_all_conditions)!=0
...
## Check that there are rows in rows5, meaning we have grabbed the write your own FITB entries
nrow(rows5)!=0
...
## As of 8/23/23, these are all free-text responses in the TET data. 
...
## Check that the correct number of columns are included in the list to be redacted. 
length(redact_free_text_cols)==22
...
## Check that the correct number of columns are included in the list to be redacted. 
length(redact_participant_cols)==4
...
## Check that the data still includes the right amount of tables. 
length(dat)==67
...
## Kaitlyn: added "Attempt to send to unsubscribed recipient" to this piece for the TET data, as there are no phone numbers
## listed for these 4 entries, so we can ignore them. 
...
## Check that the right amount of tables are redacted. 
length(redacted_tbls)==14
