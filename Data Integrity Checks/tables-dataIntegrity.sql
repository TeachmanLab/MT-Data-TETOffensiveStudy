-- ------------------------------------------------------------------------------------------------------------------------------------------------------
-- select count(distinct(study_id)) as freq, count(distinct session_name) from task_log where task_name = 'credibility'
--     and study_id in (select id from study where study_extension = 'GIDI' and id in (select study_id from participant where test_account = 0 and admin = 0)); 

#check for duplicaiton in taskLog/table
-- SELECT  study_id, session_name, COUNT(*)
-- 	FROM
-- 		task_log
-- 	where 
...
-- ------------------------------------------------------------------------------------------------------------------------------------------------------
-- select count(distinct(study_id)) as freq,  count(distinct session_name) from task_log where task_name = 'demographics'
-- and study_id in (select id from calm.study where study_extension = 'TET' and id in (select study_id from participant where test_account = 0 and admin = 0)) ;
...
-- 			and study_id in (select id from calm.study where study_extension = 'GIDI' and id in (select study_id from participant where test_account = 0 and admin = 0)) 
-- 	GROUP BY
-- 		study_id, session_name
-- 	HAVING 
-- 		COUNT(*) > 1
...
-- 		COUNT(*) > 1
-- ------------------------------------------------------------------------------------------------------------------------------------------------------
select count(distinct(study_id)) as freq, session_name from task_log where task_name = 'MentalHealthHistory'
    and study_id in (select id from calm.study where study_extension = 'TET' and id in (select study_id from participant where test_account = 0 and admin = 0)) ;
...
-- SELECT participant_id, session, COUNT(*) FROM affect where participant_id in (select id from participant where study_id in (select id from study where study_extension = 'GIDI' or study_extension = 'TET') and test_account = 0 and admin = 0  and active = 1) GROUP BY participant_id, session HAVING COUNT(*) > 1;

-- ------------------------------------------------------------------------------------------------------------------------------------------------------
select * from study where id in (select distinct(study_id) from task_log where task_name = 'credibility'
    and study_id in (select id from calm.study where study_extension = 'TED' and id in (select study_id from participant where test_account = 0 and admin = 0)) 
    and study_id not in (select distinct(study_id) from task_log where task_name = 'Wellness'
    and study_id in (select id from calm.study where study_extension = 'TED' and id in (select study_id from participant where test_account = 0 and admin = 0)) ));
